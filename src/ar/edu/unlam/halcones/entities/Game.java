	...
	public Pair<Boolean, String> checkEndgame(String action, String thing) {
		...
		for (EndGame endGame_IT : endGames) {
			...
			if (endGame_IT.getAction().contains("state")) {
				// si es un npc iteramos los npc disponibles
				if (endGame_IT.getCondition().toLowerCase().contentEquals("npc")) {
				...
				} else if (endGame_IT.getCondition().toLowerCase().contentEquals("item")) {// si es un item iteramos los
																							// item disponibles
